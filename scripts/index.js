...
function getPieChartFromJson(sheetJson, filename) {
    ...
    let items = [];
    /* 学历统计 */
    // const listArr = ['小学', '初中', '高中', '中专', '大专', '本科', '研究生', '博士', '其他']
    // for (let i in sheetJson) {
    //     if (sheetJson[i]['年龄'] <= 30) { /* 过滤器 */
    //         items.push(sheetJson[i]['学历']);
    //     }
    //     // items.push(sheetJson[i]['学历']);
    // }
    /* 性别统计 */
    const listArr = ['男', '女', '其他']
    for (let i in sheetJson) {
        if (sheetJson[i]['年龄'] <= 30) { /* 过滤器 */
            items.push(sheetJson[i]['性别']);
        }
        // items.push(sheetJson[i]['性别']);
    }
    let showMap = new Map()
    ...
    for (let oriItem of items) {
        const item = oriItem.trim()
        showMap.set(item, showMap.get(item) + 1)
    }
    // 获取所有数据
    let sheetData = [];
    for (let [key, value] of showMap.entries()) {
        sheetData.push({'name': key, 'value': value });
    }
    // 构造series要的数据
    let datas = {};
